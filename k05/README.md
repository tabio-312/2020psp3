...
## ソースコードの説明
109~111: int型の配列、変数の宣言。
113: visitを初期化する。
117: スタックを初期化する。
118: スタックに出発点をプッシュする。
119: 「深さ優先探索で到達した駅」を出力。
121: スタックが空でない間、122行目から129行目を繰り返す。
122: スタックから値をポップし、valに代入する。
123: 行ったことがない場所なら124行目から127行目を実行する。
124: visitを1にする。
125: その場所の駅名を出力する。
126: countに1を加える。
127: その場所から行ける場所をスタックにプッシュする。
134: countとsizeが等しいなら「深さ優先探索ですべての駅に到達可能」と出力。
...
211: キューに出発点を入れる。
212: 「幅優先探索で到達した駅」を出力。
214: キューが空でない間、215行目から222行目を繰り返す。
215: キューから値を取り出し、valに代入する。
216: 行ったことがないところなら217行目から222行目を実行する。
217: visitedを1にする。
218: その場所の駅名を出力。
219: countに1を加える。
220: その場所から行ける場所をキューに入れる。
227: countとsizeが等しいなら「幅優先探索ですべての駅に到達可能」と出力。
